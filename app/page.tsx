...
export default function MeetFlow() {
  ...
  function toggleMySlot(day: number, hour: number) {
    ...
    const currentlyFree = me.availability.includes(s);
    if (!currentlyFree) {
      const blocking = meetingsBlockingSlotForMember(meetings, "me", s);
      if (blocking.length > 0) {
        setScheduleBlockingMeetings(blocking);
